...
//               <div style={{ fontFamily: monospace, fontSize: 20, color: fg, lineHeight: 1.5, marginBottom: 6, textAlign: 'center' }}>
...
//               {/* Nama mengirim amount */}
//               <div style={{ fontSize: 20, color: fg, lineHeight: 1, textAlign: 'center' }}>
...
const isYouTubeLiveUrl = (url) => !!url && /youtube\.com\/live\//i.test(url);
...
const getYouTubeEmbedUrl = (url, startSeconds = 0) => {
  ...
  const liveMatch = url.match(/youtube\.com\/live\/([\w-]+)/);
  if (liveMatch) return `https://www.youtube.com/embed/${liveMatch[1]}?autoplay=1&mute=0&controls=0`;
...
const detectMediaType = (url, mediaType) => {
  ...
  if (url.includes('youtube.com/embed/') || url.includes('youtube-nocookie.com/embed/')) return 'youtube';
  if (
    url.match(/youtube\.com\/watch\?v=/) || url.match(/youtu\.be\//) ||
    url.match(/youtube\.com\/shorts\//) || url.match(/youtube\.com\/live\//)
  ) return 'youtube';
  if (isTikTokUrl(url)) return 'tiktok';
  if (mediaType === 'video') return 'video';
  if (mediaType === 'image') return 'image';
...
const renderAmountOrItem = (alert, hl, style = {}) => {
  if (alert?.donationItem?.name) {
    const qty = alert.donationItem.quantity || 1;
    return <span style={style}>{alert.donationItem.emoji || '🎁'} {alert.donationItem.name}{qty > 1 ? ` ×${qty}` : ''}</span>;
  }
...
const MediaShareOverlay = () => {
  ...
  const handleMediaDonation = useCallback((data) => {
    console.log(`[MediaShare] 📥 Media Donation Diterima!`, data.donorName, 'Rp', data.amount);
    if (configRef.current?.overlayEnabled === false) return;
  ...
  useEffect(() => {
    ...
    async function init() {
      // Channel mediashare terpisah dari channel utama — token diberi suffix -mediashare
      // sama seperti room Socket.io lama.
      const mediaChannelName = `${token}-mediashare`;

      try {
        const res = await fetch(`${API_URL}/api/overlay/pending-queue/${mediaChannelName}?events=new-media-donation`);
        const data = await res.json();
        if (!cancelled && Array.isArray(data.items) && data.items.length > 0) {
          handleMediaDonation(data.items[data.items.length - 1].data); // ambil yang terbaru saja
        }
      } catch (err) {
        console.error('[MediaShare] Gagal ambil pending-queue:', err);
      }

      ably = new Ably.Realtime({ authUrl: `${API_URL}/api/overlay/ably-token?token=${token}` });
      channel = ably.channels.get(mediaChannelName);
      const mainChannel = ably.channels.get(token);

      channel.subscribe('new-media-donation', (message) => handleMediaDonation(message.data));
      mainChannel.subscribe('settings-updated', () => loadActiveConfig('realtime'));
  ...
  }, [token, loadActiveConfig, handleMediaDonation]);
  ...
  const animVariants = {
    ...
    'slide-left': {
      initial: { x: -80, opacity: 0 }, animate: { x: 0, opacity: 1, transition: { duration: 0.4, ease: 'easeOut' } },
      exit: { x: -60, opacity: 0, transition: { duration: 0.3 } },
    },
    'slide-right': {
      initial: { x: 80, opacity: 0 }, animate: { x: 0, opacity: 1, transition: { duration: 0.4, ease: 'easeOut' } },
  ...
  const renderInner = () => {
      ...
      return (
        <div style={{ width: '100%', aspectRatio: t === 'tiktok' ? '9/16' : '16/9', overflow: 'hidden', background: '#000', borderBottom: pixelBorder, position: 'relative', zIndex: 2 }}>
          ...
          {t === 'tiktok' && (() => {
            const streamUrl = `${API_URL}/api/midtrans/tiktok-stream?url=${encodeURIComponent(alert.mediaUrl)}`;
            return (
              <video key={streamUrl} src={streamUrl} autoPlay loop muted={false} playsInline
                style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={() => setMediaError(true)} />
            );
          })()}
          {t === 'video' && (
            <video ref={videoRef} src={alert.mediaUrl} autoPlay loop muted
        ...
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '12px 14px', position: 'relative', zIndex: 2 }}>
            <div style={{ fontFamily: monospace, fontSize: 20, color: fg, lineHeight: 1.5, marginBottom: 6, textAlign: 'center' }}>
              <span style={{ fontWeight: 500 }}>{alert.donorName}</span>
              <span> mengirim </span>
              {renderAmountOrItem(alert, hl, { fontWeight: 500, color: hl, textShadow: `0 0 10px ${hl}55` })}
            </div>
            {alert.message && (
              <div style={{ fontFamily: monospace, fontSize: 18, color: fg, fontWeight: 600, lineHeight: 1.5, maxWidth: 500, textAlign: 'center' }}>
                {alert.message}
              </div>
            )}
            <div style={{ height: 3, background: hl + '25', borderRadius: 99, overflow: 'hidden', marginTop: 10 }}>
        ...
        <div style={{ fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
          ...
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10, fontFamily: "'Inter', sans-serif" }}>
            ...
            {alert.message && (
              <div style={{
                fontFamily: monospace, fontSize: 18, color: fg, fontWeight: 600,
                background: 'rgba(255,255,255,0.04)', border: dimBorder, textAlign: 'center',
                padding: '4px 10px', lineHeight: 1.5, maxWidth: 500, borderRadius: 8,
              }}>
                {alert.message}
              </div>
    ...
    if (theme === 'gifCard') {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', justifyContent: 'center', alignItems: 'center' }}>
          {mediaBlock}
          <div style={{ padding: '10px 12px', display: 'flex', textAlign: 'center', flexDirection: 'column', gap: 7 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 20, width: 'max-content', fontWeight: 500, color: 'white', borderBottom: `1px solid ${hl}25` }}>
                {alert.donorName} mengirim
              </div>
              {renderAmountOrItem(alert, hl, {
                fontFamily: "'Inter', sans-serif", fontSize: 20, marginLeft: 5, fontWeight: 500,
                color: hl, letterSpacing: '-0.5px', lineHeight: 1, textShadow: `0 0 10px ${hl}55`,
              })}
            </div>
            {alert.message && (
              <div style={{
                fontFamily: "'Inter', sans-serif", fontSize: 18, color: 'black', fontWeight: 400,
                minWidth: 350, maxWidth: 500, borderRadius: 10, background: 'white',
                border: `1px solid ${hl}25`, padding: '5px 8px', lineHeight: 1.5,
              }}>
                {alert.message}
              </div>
            )}
  ...
  if (!config) return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'transparent', gap: 12, color: 'rgba(255,255,255,0.5)', fontFamily: "'Inter', sans-serif" }}>
      <span style={{ fontSize: 32 }}>📶</span>
      <span style={{ fontSize: 18 }}>Menghubungkan...</span>
    </div>
  );
  ...
  return (
    <div style={{ width: '480px', height: 'max-content', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', overflow: 'hidden' }}>
      <audio ref={audioRef} />
      <AnimatePresence>
        {alert && (
          <motion.div
            ...
            style={{
              ...
              color: fg, width: '100%', borderRadius: 20,
