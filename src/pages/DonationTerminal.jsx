...
const StatCard = ({ label, value, icon, color }) => (
  <div className="bg-white/30 dark:bg-slate-900/60 backdrop-blur-sm border border-slate-100 dark:border-slate-800 rounded-lg p-3 md:p-4 flex flex-col gap-2">
    <div className={`w-8 h-8 rounded-lg hidden md:flex items-center justify-center ${color}`}>
      {icon}
    </div>
    <div>
      <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 md:mb-0.5">{label}</p>
...
const DonationTerminal = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex mb-5 bg-white/30 dark:bg-slate-900/60 backdrop-blur-sm border border-slate-100 dark:border-slate-800 rounded-lg p-4 md:p-5 text-white relative overflow-hidden">
      ...
      {/* ── Stats ── */}
      <div className="grid grid-cols-2 md:px-0 px-4 md:grid-cols-7 gap-2 md:gap-3">
      ...
      <div className="bg-white/30 dark:bg-slate-900/60 backdrop-blur-sm rounded-lg border border-slate-100 dark:border-slate-800 p-4 space-y-3">
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setAutoRefresh(v => !v)}
            className={`${inputClass} cursor-pointer hidden md:flex items-center gap-2 ${autoRefresh ? 'bg-green-100 text-green-600 dark:bg-green-950/40 dark:text-green-400 border-green-200 dark:border-green-800' : ''}`}>
            {autoRefresh ? <Pause size={13} /> : <Play size={13} />} Auto
          </button>

          <button onClick={() => refetch()}
            className={`${inputClass} cursor-pointer hidden md:flex items-center gap-2`}>
        ...
        <div className="flex flex-wrap gap-2 items-center justify-between">
          <div className="flex gap-1.5 flex-wrap">
            {statusFilters.map(f => (
              <button key={f.val} onClick={() => setStatusFilter(f.val)}
                className={`cursor-pointer active:scale-[0.99] px-3.5 py-2 rounded-lg font-black text-[11px] transition-all ${
          ...
          <div className="flex items-center justify-between flex-wrap gap-2 pt-1">
            <div className="grid grid-cols-4 w-full md:flex items-center gap-2">
              {/* <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Rows</span> */}
              <div className="flex gap-1 w-full">
                {[25, 50, 100, 200].map(n => (
                  <div key={n} onClick={() => setLimit(n)}
                    className={`md:w-max w-full px-4 cursor-pointer active:scale-[0.99] py-2 rounded-lg font-black text-[11px] transition-all ${
