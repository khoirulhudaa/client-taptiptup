    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          maxLength={120}
          className="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg font-bold text-sm text-slate-900 dark:text-slate-100 outline-none focus:border-blue-500 transition-all"
        />
      ...
      <div>
        ...
        <textarea
          ...
          rows={5}
          maxLength={2000}
          className="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg font-medium text-sm text-slate-900 dark:text-slate-100 outline-none focus:border-blue-500 transition-all resize-y"
        />
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {Object.entries(TYPE_CONFIG).map(([key, cfg]) => (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 text-xs font-black transition-all cursor-pointer active:scale-[0.98] ${
                  form.type === key
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <button
              ...
              className={`w-full flex items-center justify-between gap-2 px-4 py-3 rounded-lg border-2 font-black text-sm transition-all cursor-pointer active:scale-[0.97] ${
                form.isActive
            ...
            >
              ...
              <div className={`w-10 h-5 rounded-lg relative ${form.isActive ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`}>
                <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-lg shadow transition-all ${form.isActive ? 'left-5' : 'left-0.5'}`} />
              </div>
            </button>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold text-slate-900 dark:text-slate-100 outline-none focus:border-blue-500 transition-all"
            />
            {form.expiresAt && (
          ...
          <div>
            ...
            {preview ? (
              <div className="relative group">
                <img src={preview} alt="preview" className="w-full h-32 object-cover rounded-lg border-2 border-slate-200 dark:border-slate-700" />
                <button
                  onClick={removeImg}
                  className="cursor-pointer absolute top-1.5 right-1.5 w-7 h-7 bg-red-600 text-white rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                >
                  <X size={14} />
                </button>
              </div>
            ) : (
              <label className="cursor-pointer block border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-lg p-4 text-center hover:border-blue-400 transition-all">
      ...
      <div className="flex gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
        <button onClick={onClose} className="cursor-pointer flex-1 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-lg font-black text-sm hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
          Batal
        </button>
        <button onClick={handleSubmit} disabled={loading}
          className="cursor-pointer flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-black text-sm transition-all disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        {showForm && (
          <div className="fixed inset-0 z-[999] flex items-start justify-end p-0 md:py-0 overflow-y-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.97 }}
              className="relative w-full max-w-2xl min-h-screen bg-white dark:bg-slate-900 rounded-lg shadow-2xl border border-slate-100 dark:border-slate-800 z-10"
        ...
        {deleteId && (
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-lg p-8 text-center shadow-2xl border border-slate-100 dark:border-slate-800 z-10"
              onClick={e => e.stopPropagation()}
            >
              <div className="w-16 h-16 mx-auto mb-5 bg-red-100 dark:bg-red-950/40 rounded-lg flex items-center justify-center">
              ...
              <div className="flex gap-3">
                <button onClick={() => setDeleteId(null)} className="cursor-pointer flex-1 py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg font-black text-sm">Batal</button>
                <button onClick={handleDelete} disabled={deleting}
                  className="cursor-pointer flex-1 py-3.5 bg-red-600 hover:bg-red-700 text-white rounded-lg font-black text-sm transition-all disabled:opacity-60">
      ...
      <div className="gap-2 px-4 md:px-0 my-5">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-2">
          <button onClick={() => setTypeFilter('')}
            className={`px-3 py-2 rounded-lg text-[11px] font-black border transition-all cursor-pointer ${!typeFilter ? 'bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900 border-transparent' : 'bg-white dark:bg-slate-800 text-slate-400 border-slate-200 dark:border-slate-700'}`}>
            Semua Jenis
          </button>
          {Object.entries(TYPE_CONFIG).map(([key, cfg]) => (
            <button key={key} onClick={() => setTypeFilter(k => k === key ? '' : key)}
              className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-black border transition-all cursor-pointer ${typeFilter === key ? cfg.badge + ' border-current' : 'bg-white dark:bg-slate-800 text-slate-400 border-slate-200 dark:border-slate-700'}`}>
              {cfg.icon} {cfg.label}
            </button>
          ))}
        </div>
        ...
        <div className='w-full grid gap-2 border-t border-slate-200/20 pt-2 md:border-t-none grid-cols-3'>
          {[{ v: '', l: 'Semua Status' }, { v: 'true', l: '✅ Aktif' }, { v: 'false', l: '○ Non-aktif' }].map(f => (
            ...
            className={`px-3 py-2 rounded-lg text-[11px] font-black border transition-all cursor-pointer ${activeFilter === f.v ? 'bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900 border-transparent' : 'bg-white dark:bg-slate-800 text-slate-400 border-slate-200 dark:border-slate-700'}`}>
              {f.l}
            </button>
          ))}
        </div>
      ...
      <div className={`relative space-y-0 w-full md:space-y-3 grid grid-cols-1 ${isLoading || announcements.length === 0 ? 'md:grid-cols-1' : 'md:grid-cols-2'} gap-3`}>
        {isLoading ? (
          <div className={`w-full grid grid-cols-1 md:grid-cols-2 gap-3 animate-pulse`}>
            {[...Array(6)].map((_, i) => (
              <div key={i} className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden h-[200px]">
                {/* top accent */}
                <div className="h-[3px] bg-slate-200 dark:bg-slate-700" />
                <div className="p-4 md:p-5 flex items-start gap-4">
                  {/* icon */}
                  <div className="w-9 h-9 flex-shrink-0 bg-slate-200 dark:bg-slate-700 rounded-lg" />
                  {/* content */}
                  <div className="flex-1 space-y-2.5">
                    <div className="h-4 bg-slate-200 dark:bg-slate-700 rounded-lg" style={{ width: `${50 + i * 7}%` }} />
                    <div className="h-3 bg-slate-100 dark:bg-slate-800 rounded-lg w-full" />
                    <div className="h-3 bg-slate-100 dark:bg-slate-800 rounded-lg" style={{ width: '75%' }} />
                    {/* meta row */}
                    <div className="flex items-center gap-3 mt-1">
                      <div className="h-2.5 w-24 bg-slate-100 dark:bg-slate-800 rounded-lg" />
                      <div className="h-2.5 w-20 bg-slate-100 dark:bg-slate-800 rounded-lg" />
                      <div className="h-2.5 w-16 bg-slate-100 dark:bg-slate-800 rounded-lg" />
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute bottom-4 right-4 flex items-center gap-2">
                  ...
                  <div className="h-7 w-8 bg-slate-100 dark:bg-slate-800 rounded-lg" />
                  <div className="h-7 w-8 bg-slate-100 dark:bg-slate-800 rounded-lg" />
                </div>
              </div>
            ))}
          ...
          <div className="py-20 text-center text-slate-400 bg-white/30 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800 rounded-lg">
              ...
              >
                ...
                <div className="p-4 md:p-5">
                  <div className="flex items-start gap-4">
                    {/* Icon */}
                    <div className={`relative top-1 w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${cfg.color} text-white`}>
                      {cfg.icon}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isExpanded && ann.imageUrl && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-3">
                          <img src={ann.imageUrl} alt="preview" className="max-h-48 object-cover rounded-lg border border-slate-200 dark:border-slate-700" />
                        </motion.div>
                      )}
