...
export const VoiceRecorder = ({ onVoiceReady, maxSeconds = 60, disabled = false }) => {
  ...
  if (phase === 'idle') {
    return (
      ...
      >
        <p className='gap-2 flex relative top-[1.2px]'>
  ...
  if (phase === 'recording') {
    ...
    return (
      <div className="rounded-xl border-2 border-red-400 dark:border-red-600 bg-red-50 dark:bg-red-950/30 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <motion.span
              animate={{ opacity: [1, 0.2, 1] }}
              transition={{ duration: 1, repeat: Infinity }}
              style={{ width: 10, height: 10, borderRadius: 2, background: '#ef4444', display: 'inline-block' }}
            />
        ...
        {/* Progress bar */}
        <div className="h-1.5 bg-red-100 dark:bg-red-900/40 rounded-xl overflow-hidden">
          <motion.div
            animate={{ width: `${pct}%` }}
        ...
        <div className="flex gap-2">
          <button
            ...
            className="cursor-pointer flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-black text-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
          >
            ⏹ Berhenti
          </button>
          <button
            ...
            className="cursor-pointer px-4 py-2.5 bg-red-100 dark:bg-red-900/30 text-red-500 rounded-xl font-black text-sm hover:bg-red-200 active:scale-[0.98] transition-all"
          >
            ✕ Batal
          </button>
        </div>
      ...
      <div className="rounded-xl border-2 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30 p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={uploadVoice}
            className="cursor-pointer flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-black text-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
          >
            ✓ Pakai Suara Ini
          </button>
          <button
            type="button"
            onClick={discardRecording}
            className="cursor-pointer px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-xl font-black text-sm hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-[0.98] transition-all"
          >
            🔄 Ulang
          </button>
        </div>
  ...
  if (phase === 'uploading') {
    return (
      <div className="rounded-xl border-2 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30 p-4 flex items-center gap-3">
        <div className="w-5 h-5 border-2 border-blue-400 border-t-transparent rounded-full animate-spin flex-shrink-0" />
        <span className="text-xs relative top-[1.5px] font-black text-blue-600 dark:text-blue-400">Mengupload suara...</span>
      </div>
    );
  }
  ...
  if (phase === 'done') {
    return (
      <motion.div
        ...
        className="rounded-xl border-2 border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/30 p-4 space-y-2"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
  ...
  if (phase === 'error') {
    return (
      <div className="rounded-xl border-2 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/20 p-4 space-y-2">
