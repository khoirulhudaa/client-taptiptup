...
const SongOverlay = () => {
  ...
  // ── Load config (dipakai ulang untuk initial load, realtime update, reconnect, dan polling) ──
  const loadConfig = useCallback((source = 'initial') => {
    if (!token) return;
    fetch(`${SERVER_URL}/api/overlay/config/${token}?slot=A&t=${Date.now()}`)
      .then(r => r.json())
      .then(d => {
        console.log(`[SongOverlay] Config dimuat (from ${source})`);
        setConfig(d);
      })
      .catch(() => {});
  }, [token]);
  ...
  useEffect(() => {
    ...
    async function init() {
      // Ambil dulu event yang mungkin terlewat sebelum koneksi realtime tersambung
      try {
        const res = await fetch(`${SERVER_URL}/api/overlay/pending-queue/${token}?events=new-song-request,new-donation`);
        const data = await res.json();
        if (!cancelled && Array.isArray(data.items)) {
          for (const item of data.items) {
            handleIncomingSong(item.data);
          }
        }
      } catch (err) {
        console.error('[SongOverlay] Gagal ambil pending-queue:', err);
      }

      ably = new Ably.Realtime({ authUrl: `${SERVER_URL}/api/overlay/ably-token?token=${token}` });
      channel = ably.channels.get(token);

      channel.subscribe('new-song-request', (message) => {
        console.log('[SongOverlay] 🎵 new-song-request diterima:', message.data);
        handleIncomingSong(message.data);
      });

      channel.subscribe('song-skip', () => {
        console.log('[SongOverlay] ⏭️ Skip dari dashboard');
        handleSkip();
      });

      channel.subscribe('new-donation', (message) => {
        if (message.data?.songData?.videoId) {
          console.log('[SongOverlay] Fallback new-donation song');
          handleIncomingSong(message.data);
        }
      });

      channel.subscribe('settings-updated', () => loadConfig('realtime'));
      ably.connection.on('connected', () => loadConfig('reconnect'));
    }

    init();

    const polling = setInterval(() => loadConfig('polling'), 2000);
