      ...
      <div style={{
        ...
        <div style={{ position: 'relative', display: 'inline-block' }}>
          ...
          <div style={{
            ...
            {time.split('').map((ch, i) =>
              ch === ':' ? <span key={i} className="seg-colon">{ch}</span> : <span key={i}>{ch}</span>
            )}
          </div>
        </div>
      </div>
...
const SubathonWidget = () => {
  ...
  useEffect(() => {
    ...
    const channel = ably.channels.get(token);

    channel.subscribe('subathon-updated', (message) => {
      setTimer(message.data);
      setDisplaySeconds(message.data.currentSeconds || 0);
    });
