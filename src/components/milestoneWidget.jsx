...
//             const achieved = m.reached ?? (m.currentAmount || 0) >= m.targetAmount;
//             return (
//               <div key={`${m._id || ''}-${i}`} style={{ background: bgcolor ? `#${bgcolor}` : 'rgba(15,15,25,1)', borderRadius: 18, padding: '20px 30px', boxShadow: '0 4px 20px rgba(0,0,0,0.4)', textAlign: 'center' }}>
...
export const Miles1 = ({ displayList, totalDonation, color, bgcolor, textcolor, width = 600 }) => {
  const tc = textcolor ? `#${textcolor}` : '#ffffff';
  const scale = width / BASE_WIDTH;
  const getBarBg = (achieved) => {
    if (achieved) return '#10b981';
  ...
  return (
    <div style={{ width, fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      <div style={{ transformOrigin: 'top left', transform: `scale(${scale})`, width: BASE_WIDTH }}>
        <div style={{ background: bgcolor ? `#${bgcolor}` : 'rgba(15,15,25,1)', borderRadius: 20, padding: '18px 18px', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, position: 'relative', left: '-2px' }}>
            <span style={{ fontSize: 23 }}>🎯</span>
            <span style={{ fontSize: 20, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: tc }}>Milestones</span>
            <span style={{ marginLeft: 'auto', fontSize: 22, fontWeight: 800, color: tc }}>
              Rp {Number(totalDonation).toLocaleString('id-ID')}
            </span>
          </div>
              ...
              return (
                <div key={`${m._id || ''}-${i}`}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', margin: '4.5px 0px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {achieved && <span style={{ fontSize: 20 }}>✅</span>}
                      <span style={{ fontSize: 22, fontWeight: 700, color: tc, textDecoration: achieved ? 'line-through' : 'none' }}>{m.title}</span>
                    </div>
                    <span style={{ fontSize: 20, fontWeight: 800, color: achieved ? '#10b981' : tc }}>
                      {achieved ? '✓' : `${pct}%`}
                    </span>
                  ...
                  <div style={{ gap: 10, marginTop: 15, display: 'flex', justifyContent: 'end', alignItems: 'center', fontSize: 20, fontWeight: 600, color: tc }}>
                    <Target size={20} style={{ position: 'relative', top: '0.6px' }} />
                    <span>Rp {Number(m.targetAmount).toLocaleString('id-ID')}</span>
                  </div>
                </div>
              );
            })}
        ...
        {displayList.map((m, i) => {
          ...
          return (
            <div key={`${m._id || ''}-${i}`} style={{ background: bgcolor ? `#${bgcolor}` : 'rgba(15,15,25,1)', borderRadius: 18, padding: '20px 30px', boxShadow: '0 4px 20px rgba(0,0,0,0.4)', textAlign: 'center' }}>
              ...
              <div style={{ position: 'relative', height: 40, background: 'rgba(255,255,255,0.1)', borderRadius: 8, overflow: 'hidden', margin: '20px 0px' }}>
                <div style={{ height: '100%', width: `${pct}%`, background: getBarBg(achieved), borderRadius: 4, transition: 'width 1s ease' }} />
                <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800, color: achieved ? '#10b981' : tc, textShadow: '0 1px 3px rgba(0,0,0,0.8)', top: '-2px' }}>
                  {achieved ? 'Tercapai! ✓' : `${pct}%`}
                </span>
              </div>
              <div style={{ marginTop: 14, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 6 }}>
...
const MilestonesWidget = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        setMilestones(data.milestones || []);
        setTotalDonation(data.totalPaid || 0);
      }
    } catch (err) {
      console.error('Failed to fetch milestones');
    }
  ...
  // Subscribe ke channel utama (new-donation, milestones-updated) DAN channel
  // mediashare (new-media-donation) — sama seperti 2 room yang dulu di-join.
  useEffect(() => {
    if (!token) return;
    ...
    mainChannel.subscribe('new-donation', fetchData);
    mainChannel.subscribe('milestones-updated', fetchData);
    mediaChannel.subscribe('new-media-donation', fetchData);
  ...
  return (
    <>
      ...
      `}</style>
      {theme === 'miles1'
        ? <Miles1 displayList={displayList} totalDonation={totalDonation} color={color} bgcolor={bgcolor} textcolor={textcolor} width={width} />
        : <Miles2 displayList={displayList} totalDonation={totalDonation} color={color} bgcolor={bgcolor} textcolor={textcolor} width={width} />
      }
    </>
  );
};
